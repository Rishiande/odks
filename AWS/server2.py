...
if 'selected_server' not in st.session_state:
    st.session_state['selected_server'] = "Server 2"
if 'selected_project' not in st.session_state:
    st.session_state['selected_project'] = "02 TN Landscape Survey"
if 'selected_form' not in st.session_state:
    st.session_state['selected_form'] = "139-Srirangam"

# --- Email configuration ---
SMTP_SERVER = "smtp.gmail.com"
SMTP_PORT = 587
EMAIL_ADDRESS = "rishiande9999@gmail.com"
EMAIL_PASSWORD = "bwfhomcgmgxozxxt"
...
def refresh_all_forms():
    ...
    db = client['test']
    for server in forms:
        base_url_prefix = "https://tnodk02.indiaintentions.com"
        for project in forms[server]:
            for form_name, form_info in forms[server][project].items():
                project_id = form_info['project_id']
                form_id = form_info['form_id']
...
def main_dashboard():
    ...
    filtered_df = filtered_df[filtered_df['submission_date'] == today_date]
    if filtered_df.empty:
        st.warning(f"No data for today ({today_date}) in form {form_title}.")
    ...
    with col1:
        ...
        if 'submission_hour' in filtered_df.columns and not filtered_df.empty:
            ...
            fig = px.bar(hourly_counts, x='Time', y='Count',
                         title=f'Submissions per Hour on {today_date}',
            ...
            if traces:
                updatemenus = [dict(
                    buttons=[dict(
                        ...
                        args=[{'visible': [s == opt for opt in submitter_options]},
                              {'title': f"Submissions per Hour ({s if s == 'All' else f'by {s}'}) on {today_date}"}]
                ...
                fig_user = dict(
                    data=traces, layout=dict(
                        ...
                        title=f"Submissions per Hour (All Users) on {today_date}",
    ...
    with col1:
        ...
        if 'Gender' in filtered_df.columns:
            ...
            fig_gender = px.pie(gender_counts, names='Gender', values='Count',
                                title=f'Gender Distribution on {today_date}')
            ...
            if traces_gender:
                updatemenus_gender = [dict(
                    buttons=[dict(
                        label=s, method='update',
                        args=[{'visible': [s == opt for opt in submitter_options]},
                              {'title': f"Gender Distribution ({s if s == 'All' else f'by {s}'}) on {today_date}"}]
                    ) for s in submitter_options],
                    direction='down', showactive=True, x=1.15, xanchor='right', y=1.2, yanchor='top'
                )]
                fig_gender_user = dict(
                    data=traces_gender, layout=dict(
                        ...
                        title=f"Gender Distribution (All Users) on {today_date}",
                        showlegend=True
                    )
                )
    ...
    with col1:
        ...
        if 'Age_Group' in filtered_df.columns:
            ...
            fig_age = px.bar(age_counts, x='Age Group', y='Count',
                             title=f'Age Distribution on {today_date}')
            ...
            if traces_age:
                updatemenus_age = [dict(
                    buttons=[dict(
                        label=s, method='update',
                        args=[{'visible': [s == opt for opt in submitter_options]},
                              {'title': f"Age Distribution ({s if s == 'All' else f'by {s}'}) on {today_date}"}]
                    ) for s in submitter_options],
                    direction='down', showactive=True, x=1.15, xanchor='right', y=1.2, yanchor='top'
                )]
                fig_age_user = dict(
                    data=traces_age, layout=dict(
                        ...
                        title=f"Age Distribution (All Users) on {today_date}",
    ...
    with col1:
        ...
        if 'Caste' in filtered_df.columns:
            ...
            fig_caste = px.bar(caste_counts, x='Caste', y='Count',
                               title=f'Caste Distribution on {today_date}',
            ...
            if traces_caste:
                updatemenus_caste = [dict(
                    buttons=[dict(
                        label=s, method='update',
                        args=[{'visible': [s == opt for opt in submitter_options]},
                              {'title': f"Caste Distribution ({s if s == 'All' else f'by {s}'}) on {today_date}"}]
                    ) for s in submitter_options],
                    direction='down', showactive=True, x=1.15, xanchor='right', y=1.2, yanchor='top'
                )]
                fig_caste_user = dict(
                    data=traces_caste, layout=dict(
                        ...
                        title=f"Caste Distribution (All Users) on {today_date}",
    ...
    with col1:
        ...
        if 'Duration_Group' in filtered_df.columns:
            ...
            fig_duration = px.bar(duration_counts, x='Duration', y='Count',
                                  title=f'Survey Duration Distribution on {today_date}')
            ...
            if traces_duration:
                updatemenus_duration = [dict(
                    buttons=[dict(
                        label=s, method='update',
                        args=[{'visible': [s == opt for opt in submitter_options]},
                              {'title': f"Survey Duration Distribution ({s if s == 'All' else f'by {s}'}) on {today_date}"}]
                    ) for s in submitter_options],
                    direction='down', showactive=True, x=1.15, xanchor='right', y=1.2, yanchor='top'
                )]
                fig_duration_user = dict(
                    data=traces_duration, layout=dict(
                        ...
                        title=f"Survey Duration Distribution (All Users) on {today_date}",
    ...
    if not filtered_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"{form_title.replace(' ', '_')}_data_{today_date}.csv",
            mime="text/csv"
        )
    else:
...
def overall_dashboard():
    ...
    # Filter data for today's date
    today_date = datetime.now().strftime('%Y-%m-%d')
    filtered_df = aggregated_df[aggregated_df['submission_date'] == today_date]
    if filtered_df.empty:
    ...
    if not filtered_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"Aggregated_TNMaster_data_{today_date}.csv",
            mime="text/csv"
        )
    else:
        st.info("No data to download.")
